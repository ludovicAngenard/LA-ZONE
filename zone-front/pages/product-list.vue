<template>
  <div>
    <div
      style="width: 30%"
      class="v-input mx-auto mt-2 v-input--hide-details v-input--dense theme--light v-text-field v-text-field--single-line v-text-field--solo v-text-field--solo-flat v-text-field--is-booted v-text-field--enclosed v-text-field--placeholder rounded-lg"
      data-v-7f162986=""
    >
    ...
    <div v-if="this.$store.state.categories.filteredCategories.length != 0">
      <v-chip
        ...
        <v-col v-for="product in findSaleList()" :key="product.id" sm="4">
          ...
          >
            ...
            <a href="product" class="text-decoration-none"
              ><h4 class="text-center py-1 white--text">
                {{ product.brand }}
              </h4></a
            >
            ...
            <h4 class="text-center py-1 red--text">{{ product.price }} €</h4>
          </v-card>
        </v-col>
      </v-row>
      <cart> </cart>
...
export default {
  ...
  methods: {
    ...
    toggle_wish(evt, product) {
      if (
        evt.target.src.split("/")[evt.target.src.split("/").length - 1] ==
        "heart_empty.png"
      ) {
    ...
    removeFilter(filter) {
      ...
      this.$store.dispatch(
        "products/remove_products_from_filtered_products",
        filteredProducts
      );
    },
