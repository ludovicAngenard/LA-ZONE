<template>
  <div>
    <v-card min-height="500px" width="350px" class="text-center pb-4 mb-6">
      <v-container>
        <v-icon dark class="mb-2"> mdi-cart-variant </v-icon>
        <v-row>
          <v-col> Produit </v-col>
        ...
        >
          ...
          <v-col class="d-flex">
            <v-btn
              ...
              class="mr-2"
            >
              <v-icon dark v-on:click="sub(product)"> mdi-minus </v-icon>
            </v-btn>
            {{ product.quantity }}
            <v-btn
              fab
              ...
              class="ml-2"
            >
              <v-icon dark v-on:click="plus(product)"> mdi-plus </v-icon>
            </v-btn>
          </v-col>
      ...
      <v-card
        ...
        @dragenter.prevent
      >
        <span class="my-auto mx-auto"> Glissez vos produits au panier ! </span>
      </v-card>
      <v-row class="mt-4">
        <v-col> TOTAL : </v-col>
        <v-col class="red--text"> {{ totalPrice }} € </v-col>
      </v-row>
      <v-btn class="ma-2 mt-6" color="success" @click="valid_cart">
        Valider mon panier
      </v-btn>
    </v-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    findPrice(product) {
      ...
      if (index > -1) {
        return (
          this.$store.state.products.cartProducts[index].quantity *
          this.$store.state.products.cartProducts[index].price
        );
      }
    },
    findTotal() {
